...
GLOBAL_PROGRESS = [
    ...
    ["权限与审计", "进行中", 0.97, "请求/评论/附件按角色校验，动作写入 history；组织 scope 已废除", "他人详情粒度和团队 scope 待确认"],
...
COMPLETED_ITEMS = [
    ...
    ["2026-08-10", "Admin 管理页面全部切换真实 API", "用户、角色、权限、团队、路由、审计和 Dashboard"],
    ...
    ["2026-08-11", "废除组织管理", "移除 Organization 页面、主流程字段、前端 API/types；V1.0.29 放开 team.org_id"],
]

PENDING_CONFIRMATIONS = [
    ["CONF-001", "普通用户查看他人请求的详情字段和公开留言范围", "组织范围已废除，上线前确认字段粒度"],
...
ROLE_DATA = {
    "Admin": {
        "desc": "Admin 管理账号、权限、团队、路由、全局请求和审计，也可作为全局技术负责人。",
        "summary": [
            ["登录与导航", "进行中", 0.97, "真实 JWT、/auth/me 和角色导航", "Docker 回归"],
            ["用户/角色/权限", "进行中", 0.97, "页面和 system API 使用真实 CRUD；用户不再绑定组织", "Docker 写操作回归"],
            ["团队与路由", "进行中", 0.97, "真实成员、负责人、路由规则和自动匹配；团队不再绑定组织", "V1.0.29 实库验证"],
        ...
        "requirements": [
            ...
            requirement("ADM-002", "废除组织管理", "P0", 1.00, "删除 Organization 页面、路由、前端 API/types；主流程不再出现 organization", "旧隐藏 CMDB 文件保留历史字段", "已确认以团队为主"),
            requirement("ADM-003", "管理用户和启用/禁用", "P0", 0.97, "UserList.vue + User CRUD API；不再分配组织", "Docker 待验证", "是否增加物理删除"),
            requirement("ADM-004", "管理角色和权限", "P0", 0.96, "RoleList.vue、PermissionList.vue + 真实 API", "Docker 待验证"),
            requirement("ADM-005", "管理团队、负责人和成员", "P0", 0.97, "TeamList.vue、V1.0.25、V1.0.29、Team/User DTO", "Flyway 未实库验证", "多团队负责人 scope"),
        ...
        "pages": [["Admin Overview", "是", "进行中", "Dashboard API"], ["Users", "是", "进行中", "真实 CRUD API"], ["Roles / Permissions / Teams", "是", "进行中", "真实 CRUD API"], ["Routing Rules", "是", "进行中", "真实 Routing Rule API"], ["Requests / Audit Logs", "是", "进行中", "真实 Request/Audit API"]],
    },
    "普通用户": {
        ...
        "requirements": [
            ...
            requirement("REQ-002", "创建轻量但信息完整的新请求", "P0", 0.99, "真实 create API、富文本、时间字段、粘贴图片和附件；不再选择组织", "Docker 上传待验证"),
            ...
            requirement("REQ-005", "只读查看其他用户请求", "P0", 0.92, "组织 scope 已废除，请求读取不再按 organization 限制", "详情字段粒度未定", "描述/公开留言可见范围"),
    ...
    "技术处理人员": {
        ...
        "requirements": [
            ...
            requirement("TECH-004", "跨团队转派给其他 IT 人员", "P0", 0.97, "Assignee 显示 team + assign API", "Docker 待验证"),
    ...
    "技术负责人": {
        ...
        "requirements": [
            ...
            requirement("TL-003", "跨团队重新分配/转交", "P0", 0.97, "真实 assign API，人员显示 team", "Docker 待验证"),
