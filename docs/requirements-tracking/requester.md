...
| 新建请求 | 进行中 | 98% | 真实创建 API、富文本、粘贴图片、附件、时间字段；Type 和 Affected Service 读取 Admin Code Tables | Docker 上传与 Flyway 回归 |
...
| REQ-002 | 创建轻量但信息完整的新请求 | P0 | 进行中 | 99% | Title、Type、Affected Service、Priority、Occurrence Time、Requested Resolution Time、富文本和附件均走真实 API；Type 和 Affected Service 来自可维护 Code Table；不再要求 Organization | Docker 上传和 Flyway 待验证 | 后续继续参考同类产品优化字段体验 |
...
| REQ-005 | 只读查看其他用户的请求 | P0 | 进行中 | 92% | 组织范围已废除，请求读取不再按 organization scope 限制 | 详情字段粒度未定 | 他人描述/公开留言可见范围 |
...
| New Request | 进行中 | 真实富文本、粘贴图片、附件上传和 Admin 可维护下拉项 |
...
已确认：留言支持附件；Close note 可选；`User Test Failed` 原因可选且允许附件；FAQ 保留隐藏；创建请求不再选择组织。
